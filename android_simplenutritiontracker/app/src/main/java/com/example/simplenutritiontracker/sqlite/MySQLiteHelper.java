...
public class MySQLiteHelper extends SQLiteOpenHelper {
    ...
    private static final String DATA_BASE_NAME = "macros.sqlite";
    private static final int DATABASE_VERSION = 1;

    // Note that when getWritableDatabase() gets called (it's inherited), Android will check to see
    // if the database file (tasks.db) has been created.
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String testSql = "create table BLAH (_id INTEGER PRIMARY KEY AUTOINCREMENT, calorieIntake INTEGER, proteinMacros INTEGER," +
                " carbMacros INTEGER, fatMacros INTEGER)";

        db.execSQL(testSql);
        Log.d(TAG, testSql);

        // Insert a row into the BLAH table (for testing)
        String testInsert = "insert into BLAH (calorieIntake, proteinMacros, carbMacros, fatMacros) values (2000, 100, 200, 88);";
        db.execSQL(testInsert);
        Log.d(TAG, testInsert);*/


        String macrosTableSQL = SQLMacrosDataAccess.TABLE_CREATE;
        db.execSQL(macrosTableSQL);
        Log.d(TAG, macrosTableSQL);

        String mealTableSQL = SQLMealDataAccess.TABLE_CREATE;
        db.execSQL(mealTableSQL);
        Log.d(TAG, mealTableSQL);

        String foodGroupsTableSQL = SQLFoodGroupsDataAccess.TABLE_CREATE;
        db.execSQL(foodGroupsTableSQL);
        Log.d(TAG, foodGroupsTableSQL);

    }

    @Override
    //I assume this method gets called by Android?
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        switch(oldVersion) {
            case 1:
               ...
               String addColumnSQL = "ALTER TABLE macros ADD someNewColumn VARCHAR;";
